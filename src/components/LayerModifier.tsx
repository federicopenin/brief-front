    ...
    <div className="w-full py-6 animate-fadeIn pb-32">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {flatLayers.length === 0 ? (
          ...
          flatLayers.map((layer: FlatLayer, index: number) => (
            <LayerCard
              key={layer.id}
              layer={layer}
              ...
              {status === "success" && downloadUrls ? (
                <div className="flex gap-2">
                  <button
                    ...
                    className="py-2.5 px-3 rounded-xl text-white font-semibold text-sm flex items-center justify-center transition-all bg-zinc-700 hover:bg-zinc-600 border border-zinc-600"
                    title="Preview"
                  >
                    <svg
                      className="w-5 h-5"
      ...
      {showPreview && editedHistoryId && (
        <PreviewModal
          historyId={editedHistoryId}
          filename={data.filename}
